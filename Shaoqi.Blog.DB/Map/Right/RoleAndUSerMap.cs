...
{
    public class RoleAndUSerMap
    {
        public  RoleAndUSerMap(EntityTypeBuilder<RoleAndUSer> entityMap, string schema = "dbo")
        {
            entityMap.ToTable(schema + "Right_RoleAndUSer");
         entityMap.HasKey(a => a.FID);
            entityMap.Property(a => a.FID).HasColumnName("FID").IsRequired().HasMaxLength(50);
            entityMap.Property(a => a.RightId).HasColumnName("RightId");
